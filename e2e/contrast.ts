 ...
 *    files the node under `incomplete`. the chart panel and the DOOM
 *    lab each sit on a `linear-gradient` fading to transparent, and
 *    `.cl-hero-why` is a `color-mix(in oklab, var(--accent) 6%, transparent)`
 *    wash. Text on any of those composites onto whatever is behind rather than
 *    a flat colour.
 *  - text faded by an ancestor's `opacity` — axe reads the declared `color`,
 *    which is not the colour that lands on screen. Several elements here do
 *    that, at 0.35, 0.5 and 0.75.
 *
 ...
 *
 *  - TEXT SCROLLED OUT OF A CLIPPING ANCESTOR PAINTS NOTHING. The readout, ops and
 *    sources tables sit in horizontally scrolling wrappers, so at 380px much of
 *    them is scrolled out of the frame. Content past the client box is not dimmed or partly drawn — it is
 ...
 *    erosion chart, the DOOM visualisation and the sweep chart as real inline
 *    SVG, with axis and series labels as `<text>` siblings of the shapes they
 *    sit on — so this rule is load-bearing here. Where an `<svg>` does
...
export async function auditContrast(page: Page): Promise<ContrastFailure[]> {
  return page.evaluate(() => {
     ...
     * matters" wash is `color-mix(in oklab, ...)`. Chromium reports that
     ...
     * the farthest-corner radius. That matters here: the DOOM lab paints a
     * `linear-gradient(180deg, var(--accent-soft), transparent 120px)` — a
     * wash confined to its top 120px — so judging the table at the bottom of
     * that panel by the top stop would invent a backdrop it never sits on. A non-gradient layer (`url()`, `none`) is unmeasurable and
     * paints nothing.
     */
     ...
     *
     * This is one very long single-page document — a primer matrix of toggle
     * cells, three tables, three SVG figures and a syndrome-card grid — and
     * every text node walks the same handful of ancestors. Without
    ...
    const isVisible = (el: Element): boolean => {
      ...
      // paints nothing. This page has two native `<details>` — the sources
      // list (open by default) and the verification block (collapsed) — and
      // the gate scans both configurations of each explicitly.
     ...
     * it, the `aria-hidden` decorations on this page — the chart legend swatch
     * row chief among them — are measured as if they took the element's
     * declared `color`, which describes nothing on screen.
     */
     ...
     * lab builds its SVG figures from template strings in main.ts, so the rule
     * is a live guard here rather than a formality.
     */
