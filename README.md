...
The page has five parts: an interactive **erosion chart** (slider for D, three
live curves, the red floor line, live readout table), a **DOOM mechanism**
explainer with live syndrome counts, a **key‑rotation policy calculator**, an
...
input, same output, no `Math.random`, no `Date`, no network.
